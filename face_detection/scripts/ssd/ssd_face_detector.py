#!/usr/bin/env python3
"""
SSD Face Detector Model Evaluation Script with Highest Confidence Filtering

This script evaluates the performance of a face detection SSD model on the LFW dataset,
...
def parse_arguments():
    parser = argparse.ArgumentParser(description='Test SSD Face Detector on LFW dataset with highest confidence filtering')
    parser.add_argument('--dataset', type=str, default='data/lfw-deepfunneled/lfw-deepfunneled',
                      help='Path to the LFW dataset')
    parser.add_argument('--sample_size', type=int, default=1000,
                      help='Number of images to sample for evaluation')
    parser.add_argument('--output_dir', type=str, default='ssd_face_evaluation_results_highest_confidence',
    ...
    parser.add_argument('--image_size', type=int, nargs=2, metavar=('WIDTH', 'HEIGHT'),
                      help='Specify the size of the image as width and height (used for SSD input size)')
    return parser.parse_args()

def download_model_files():
    """
    Download the pre-trained SSD face detector model files if they don't exist.
    """
    model_dir = "Detect_models/custom_detector"
...
def load_model():
    """
    Load the SSD face detector model.
    ...
    # Path to model files
    prototxt_path = "Detect_models/custom_detector/deploy.prototxt"
    model_path = "Detect_models/custom_detector/res10_300x300_ssd_iter_140000.caffemodel"
    ...
    print(f"Loading SSD face detector model...")
    ...
    print("SSD face detector model loaded successfully")
    return model
...
def detect_faces(model, image_paths, conf_thres=0.5, filter_highest_confidence=True):
    detections = []
    times = []
    print(f"Running face detection using SSD face detector with highest confidence filtering...")
    ...
    for image_path in tqdm(image_paths):
        ...
        # SSD face detector expects 300x300 images with specific mean subtraction
        ...
        for i in range(output.shape[2]):
            ...
            if confidence >= conf_thres:
                ...
                # Ensure we have a valid box with non-zero dimensions
                if x1 < x2 and y1 < y2:
...
def print_results(metrics):
    ...
    print("\n" + "=" * 80)
    print(f"SSD FACE DETECTION EVALUATION RESULTS")
    print("=" * 80)
...
def main(args=None):
    ...
    try:
        model = load_model()
        print("Successfully loaded SSD face detector model")
    except Exception as e:
        print(f"Error loading SSD face detector model: {e}")
        sys.exit(1)
    ...
    # Calculate metrics
    metrics = evaluate_performance(detections, times, model_type="SSD Face Detector")
    ...
    try:
        visualize_results(detections, getattr(args, 'visualize', 10), args.output_dir, model_type="SSD Face Detector")
        generate_graphs(detections, times, args.output_dir, model_type="SSD Face Detector")
